...
class BrowserDetector:
    ...
    def get_dropdown_choices(self) -> List[tuple]:
        """Get choices for dropdown: [(display_text, browser_name), ...]"""
        browsers = self.detect_all_browsers()
        choices = []
        for b in browsers:
            version = f" ({b.version})" if b.version else ""
            display = f"{b.display_name}{version}"
            choices.append((display, b.name, b.path))
        return choices


if __name__ == "__main__":
    print("Detecting browsers silently...")
    detector = BrowserDetector()
    browsers = detector.detect_all_browsers()
    
    print(f"\nFound {len(browsers)} browser(s):")
    for b in browsers:
        print(f"  ✓ {b.display_name} - {b.version or 'version unknown'}")
        print(f"    Path: {b.path}")
