                ...
                item.querySelectorAll('[data-automation-id="choiceItem"]').forEach((choice, optIdx) => {
                    ...
                    if (radioSpan || radioInput) {
                        ...
                        options.push({
                            ...
                            position: posInSet,
                            selenium: {
                                // Best selector: by value (most reliable, survives reordering)
                                byValue: '[data-automation-value="' + value.replace(/"/g, '\\"') + '"]',
                                // Alt: by input value
                                byInputValue: 'input[value="' + value.replace(/"/g, '\\"') + '"]',
                                // Alt: by position (fragile if options reorder)
...
def get_questions_for_ui(driver) -> dict:
    ...
    return {
        ...
        "hasNext": data["hasNextButton"],
        "hasBack": data["hasBackButton"],
        "hasSubmit": data["hasSubmitButton"],
