...
"""
Project: DocuFlow
File: bulk_c1f2_certificados_TEST_8_withPDF.py
Created: 2025-12-09
Author: @lewopxd
...
TEST 8: Incluye conversión a PDF al final del proceso (BATCH_END)
        Usa FILE_MAP con comparador CONTAINS para renombrar los PDFs
"""
...
EXCEL_CONFIG = {
    "sheet": "COHORTE 1",
    "table": "Tabla_1",
    "range_config": {"type": "multiple_row", "data": "2-10"}  # Filas 2-10
}

# --- [ CONFIGURACIÓN PDF CONVERSION ] ---
PDF_CONVERSION_CONFIG = {
    "enabled": True,
    "method": "office",      # "office" (MS Word) | "libreoffice" (CLI)
    "timing": "BATCH_END",   # IMMEDIATE | AFTER_ROW | BATCH_END
    "keep_docx": True,       # Mantener archivos DOCX
    
    # Configuración de compresión
    "compression": {
        "enabled": True,
        "level": "high",  # low | medium | high
        "max_size_kb": None  # Sin límite de tamaño
    },
    
    # Configuración de metadata
    "metadata": {
        "author": "MARCELA",        # Vacío por defecto
        "creator": "MSWORD",
        "producer": "MSWORD",
        "custom": {}         # Campos personalizados vacíos
    },
    
    # Mapa de archivos: comparador CONTAINS para renombrar
    "file_rules": [
        {
            "comparator": "CONTAINS",
            "pattern": "ACTA 1",
    ...
    "default_rule": {"action": "SKIP"},  # Ignorar archivos que no coincidan
    "overwrite": True
}
...
if __name__ == "__main__":
    ...
    try:
        ...
        # --- [ INICIO: CONVERSIÓN A PDF (BATCH_END) ] ---
        if PDF_CONVERSION_CONFIG.get("enabled", False):
            print(f"\n{'='*70}")
            print("📄 INICIANDO CONVERSIÓN A PDF (BATCH_END)")
            print(f"{'='*70}")
            
            try:
                from assembler.jobs.word_to_pdf_job import WordToPdfJob
                
                # Construir lista de carpetas a procesar (jóvenes)
                joven_folders = [f for f in BASE_OUTPUT_PATH.iterdir() if f.is_dir()]
                print(f"📁 {len(joven_folders)} carpetas de jóvenes encontradas")
                
                total_pdfs_success = 0
                total_pdfs_failed = 0
                
                for joven_folder in joven_folders:
                    print(f"\n  📂 Procesando: {joven_folder.name}")
                    
                    # Configurar job para esta carpeta
                    pdf_job_config = {
                        "mode": "file_rules",
                        "source": {
                            "base_path": str(joven_folder),
                            "scan_mode": "ALL"
                        },
                        "file_rules": PDF_CONVERSION_CONFIG.get("file_rules", []),
                        "default_rule": PDF_CONVERSION_CONFIG.get("default_rule", {"action": "SKIP"}),
                        "overwrite_existing": PDF_CONVERSION_CONFIG.get("overwrite", True),
                        "batch_size": 10
                    }
                    
                    job = WordToPdfJob(pdf_job_config)
                    valid, error = job.validate_config()
                    
                    if not valid:
                        print(f"    ❌ Config inválida: {error}")
                        continue
                    
                    result = job.run()
                    
                    if result.data:
                        summary = result.data.get("job_summary", {})
                        success = summary.get("success_count", 0)
                        failed = summary.get("failure_count", 0)
                        total_pdfs_success += success
                        total_pdfs_failed += failed
                        
                        if success > 0:
                            print(f"    ✅ {success} PDFs generados")
                        if failed > 0:
                            print(f"    ❌ {failed} conversiones fallidas")
                
                print(f"\n{'='*70}")
                print("📄 CONVERSIÓN PDF COMPLETADA")
                print(f"{'='*70}")
                print(f"  ✅ PDFs exitosos: {total_pdfs_success}")
                print(f"  ❌ PDFs fallidos: {total_pdfs_failed}")
                
            except ImportError as e:
                print(f"❌ No se pudo importar WordToPdfJob: {e}")
                print("   Asegúrate de tener docx2pdf instalado: pip install docx2pdf")
            except Exception as e:
                print(f"❌ Error en conversión PDF: {e}")
                import traceback
                traceback.print_exc()
        # --- [ FIN: CONVERSIÓN A PDF ] ---
