...
def get_files_in_person_folder(person_folder: Path) -> List[Dict]:
    ...
    # CORRECCIÓN: Buscar dentro de la subcarpeta "FASE 3 - INTERMEDIACION"
    intermediacion_folder = person_folder / "FASE 3 - INTERMEDIACION"
    
    if not intermediacion_folder.exists():
        print(f"\n  ⚠ No se encontró la carpeta 'FASE 3 - INTERMEDIACION'")
        print(f"     Intentando buscar directamente en la carpeta de la persona...")
        search_folder = person_folder
    else:
        print(f"\n  ✓ Carpeta 'FASE 3 - INTERMEDIACION' encontrada")
        search_folder = intermediacion_folder
        
        # Listar contenido de la subcarpeta
        print(f"\n  📋 CONTENIDO DE 'FASE 3 - INTERMEDIACION':")
        try:
            sub_items = list(intermediacion_folder.iterdir())
            print(f"     Total de items: {len(sub_items)}")
    ...
    print(f"\n  🔎 Buscando archivos .docx y .pdf en: {search_folder.name}")
    
    for ext in ['*.docx', '*.pdf']:
        print(f"\n     Buscando con patrón: {ext}")
        found_files = list(search_folder.glob(ext))
...
def main():
    ...
    if global_stats['total_files_found'] == 0:
        ...
        print(f"  {'⚠' * 70}")
        print(f"\n  Posibles causas:")
        print(f"  1. Los archivos no tienen el formato: YYYYMMDD_ACTA...")
        print(f"  2. La estructura de carpetas es diferente a la esperada")
        print(f"  3. Los archivos no están en las subcarpetas de personas")
        print(f"  4. Los patrones de ACTA no coinciden con los nombres reales")
        print(f"\n  Recomendaciones:")
        print(f"  1. Verifica la estructura de carpetas mostrada arriba")
        print(f"  2. Revisa los nombres de archivo en cada carpeta de persona")
        print(f"  3. Compara los patrones de ACTA con los nombres reales")
        print(f"  4. Verifica que los archivos tengan fecha al inicio del nombre")
