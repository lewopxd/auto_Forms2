/**
 * AutoForms Login UI - Wait for user authentication
 * Floating panel in corner - does NOT block page interaction
 * Auto-reinjects on page navigation/reload
 */

(function () {
    'use strict';

    const ROOT_ID = '__autoforms_login_root__';
    ...
    function createLoginUI() {
        ...
        shadow.innerHTML = `
            ...
            <div class="panel" id="panel">
                <div class="header" id="header">
                    ...
                    <span class="header-title">AutoForms - Login</span>
        ...
        btnContinue.onclick = () => {
            window.__autoforms_commands.push({ type: 'login_done', time: Date.now() });
            host.remove();
        };

        // Cancel button - Stop automation
        btnCancel.onclick = () => {
            window.__autoforms_commands.push({ type: 'cancel', time: Date.now() });
            host.remove();
        };

        console.log('[AutoForms] Login UI ready');
    }

    // Execute
    waitForBody(createLoginUI);
})();
